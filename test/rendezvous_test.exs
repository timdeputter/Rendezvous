defmodule RendezvousTest do
  ...
  test "When a buckets fails only the objects in that bucket have to be moved", context do
    bucketsWithoutB = delete(context[:buckets], "Node B")
    ...
    assert member? bucketsWithoutB, get_hrwbucket("AA",bucketsWithoutB)
  end
  ...
  defp get_percentage_of(collection, object) do
    number_of_eq_to_object = filter(collection, fn(e) -> e == object end) |> count
    result = Float.round(100 / count(collection) * number_of_eq_to_object,0)
    result
  end

  defp get_buckets(objects,buckets) do
    map(objects, fn(object) -> get_hrwbucket(object, buckets) end)
  end

  defp get_hrwbucket(object,buckets) do
    get(:sha, object, buckets)
  end

  defp is_bucket(buckets, may_be_bucket) do
    any?(buckets, fn e -> e == may_be_bucket end)
  end

  defp all_equal?(collection) do
    uniq(collection) |> count == 1
  end
